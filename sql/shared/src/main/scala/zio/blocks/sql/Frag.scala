...
object Frag {
  ...
  extension (frag: Frag) {
    ...
    def queryOne[A](using con: DbCon, codec: DbCodec[A]): Maybe[A] = {
      ...
      try {
        ...
        try {
          ...
          val rs = ps.executeQuery()
          try {
            val result   = if (rs.next()) Maybe(codec.readValue(rs.reader, selectLabels(rs.reader, codec))) else Maybe.absent
    ...
    /** Executes this fragment as an INSERT/UPDATE/DELETE and returns the affected row count. */
