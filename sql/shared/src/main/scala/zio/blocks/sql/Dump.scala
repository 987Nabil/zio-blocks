...
object Dump {
   ...
   *
   * '''Inline-value requirement:''' the `query` argument must be
   * inline-constructible. A preconstructed non-inline val cannot expose its
   ...
   * fallback. Use `inline val`/`inline def` or inline the query at the call
   * site.
   */
