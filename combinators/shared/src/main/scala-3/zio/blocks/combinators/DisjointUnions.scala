 ...
 *
 * The `DisjointUnions` module provides a unified typeclass `DisjointUnions[L, R]` that both
 * combines an `Either[L, R]` into a union type `L | R` and separates it back.
 */
object DisjointUnions {

  trait DisjointUnions[L, R] {
    type Out
  ...
  object DisjointUnions {
    type WithOut[L, R, O] = DisjointUnions[L, R] { type Out = O }

    inline given disjointUnions[L, R](using tt: TypeTest[L | R, R]): WithOut[L, R, L | R] =
      ${ disjointUnionsMacro[L, R]('tt) }

    private def disjointUnionsMacro[L: Type, R: Type](
      ...
      if (overlap.nonEmpty) {
        ...
        report.errorAndAbort(
          s"Disjoint union types must contain unique types. Found overlapping types: $overlapNames. " +
      ...
      rRepr.dealias match {
        case _: OrType =>
          report.errorAndAbort(
            "The right type of a DisjointUnions.DisjointUnions must not be a union type. " +
              "Use a simple (non-union) type for R to ensure the separator peels exactly one type."
          )
        case _ => // ok
      }

      '{ new DisjointUnionInstance[L, R](using $tt) }
    }

    private[combinators] class DisjointUnionInstance[L, R](using tt: TypeTest[L | R, R])
        extends DisjointUnions[L, R] {
      type Out = L | R
  ...
  def combine[L, R](either: Either[L, R])(using u: DisjointUnions[L, R]): u.Out = u.combine(either)
}
